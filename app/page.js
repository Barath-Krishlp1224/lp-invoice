...
// import { Upload, FileText, Download, CheckCircle, AlertCircle, Building2, Hash, ChevronLeft, ChevronRight, Edit3, DollarSign, Copy, Search, Archive } from 'lucide-react';
...
//     // Removed [generatingPDFs] and [pdfProgress] states
...
//     // NOTE: This effect fetches JSZip, which is essential for the bulk ZIP download function.
//     useEffect(() => {
...
//             const detectedCols = detectRequiredColumns(result.headers);
...
//     const generateProfessionalInvoiceHTML = (rowData, headers, rowIndex) => {
...
//             formatCellValue(rowData[detectedCols.transactionTime], detectedCols.transactionTime).split(' ')[0] : 
//             '00:23'; 
...
//         const invoicePrefixFinal = merchantColumn ? merchantInfo.invoicePrefix : invoicePrefix;
//         const invoiceNumber = `${invoicePrefixFinal}${String(rowIndex).padStart(3, '0')}`;
...
//     // --- Renamed and Updated Function to Download ZIP of HTML Invoices ---
//     const bulkDownloadInvoices = async () => {
//         if (!preview || !preview.data.length || typeof window.JSZip === 'undefined') {
//             setError('Required data or JSZip library not loaded.');
//             return;
//         }

//         if (!rrnColumn || !upiColumn || !amountColumn || (!merchantColumn && !invoicePrefix.trim())) {
//             setError('Please ensure all required columns (RRN, UPI, Amount) and a merchant setting are selected.');
//             return;
//         }
...
//                 // Filename based on RRN
//                 const filename = `${getFilenameFromRRN(rowData)}.html`;
...
//                 // Use JSZip's internal progress for final stages
...
//             link.download = 'Lemonpay_Invoices.zip';
//             document.body.appendChild(link);
//             link.click();
//             document.body.removeChild(link);

//             // Give a final success alert
//             alert(`Successfully generated a ZIP file containing ${totalRows} HTML invoices. Open the HTML files individually in your browser and use the 'Print' > 'Save as PDF' function to generate PDFs.`);
...
//         setShowDuplicates(false);
//         setCurrentPage(1);
//         // Removed setGeneratingPDFs(false)
//         setGeneratingZip(false);
//         // Removed setPdfProgress(0)
//         setZipProgress(0);
...
//     const isReadyToGenerate = rrnColumn && upiColumn && amountColumn && (merchantColumn || invoicePrefix.trim());
...
//                 </div>
//                 <h2 className="text-4xl font-bold text-white-900 mb-3">Lemonpay Invoice Generator</h2>
//             </div>
...
//                                             {preview ? 'Data Loaded' : 'Preview Data'}
//                                         </>
//                                     )}
//                                 </button>
//                             </div>
...
//                             {/* BULK DOWNLOAD BUTTON (Renamed to 'Print & Download All Invoices') */}
//                             {isReadyToGenerate && (
//                                 <div className="mt-8 flex justify-center space-x-4">
//                                     <button
//                                         onClick={bulkDownloadInvoices}
//                                         disabled={generatingZip}
//                                         className="inline-flex items-center px-10 py-4 bg-gradient-to-r from-purple-500 to-pink-600 text-white hover:from-purple-600 hover:to-pink-700 rounded-xl font-bold text-lg shadow-xl transition-all transform hover:scale-105 disabled:opacity-50 disabled:transform-none"
//                                     >
//                                         <Archive className="w-6 h-6 mr-3" />
//                                         {generatingZip ? `Zipping... ${zipProgress.toFixed(0)}%` : `Print & Download All Invoices (${preview.data.length})`}
//                                     </button>
//                                 </div>
//                             )}
...
//                                                                 <AlertCircle className="w-3 h-3 mr-1" /> Missing fields
//                                                             </div>
//                                                          )}
//                                                     </td>
//                                                 </tr>
...
//                                 <h4 className="text-lg font-semibold text-purple-800">Generating ZIP File...</h4>
//                                 <p className="text-sm text-purple-600">Creating compressed file with all invoices as HTML documents.</p>
//                             </div>
...
//                                     ></div>
//                                 </div>
//                                 <div className="text-lg font-bold text-purple-700 min-w-[4rem]">{zipProgress.toFixed(0)}%</div>
//                             </div>
//                         </div>
//                     )}
//                 </div>
...
/**
 * The main page component acts as a Server Component wrapper 
 * for the client-side Invoice Generator.
 */
